        ...
        child: SafeArea(
          child: Center(
            child: FutureBuilder(
              ...
              future: dbRef.orderByChild("user_id").equalTo('$userId').once(),
              builder: (context, AsyncSnapshot<DataSnapshot> snapshot) {
                ...
                if (snapshot.hasData) {
                  if (snapshot.data.value is List) {
                    ...
                    print('  data in user skills ${snapshot.data.value.values}');
                  ...
                  return ListView.builder(
                    ...
                    itemBuilder: (BuildContext context, int index) {
                      ...
                      if (response.toList()[index]['status'] == 'inprogress') {
                        return Container();
                      }
                      ...
                      return FutureBuilder(
                        ...
                        builder: (context, snapshot2) {
                          if (snapshot2.hasData) {
                            print('  data in skills ${snapshot2.data.value}');
                            if (dailyChallengeLang.contains(snapshot2.data.value['name'])) {
                              String text = snapshot2.data.value['name'];
                              String level = snapshot2.data.value['level'];
                              if (snapshot2.data.value['name'] == 'go_lang') {
                                text = go;
                              } else if (snapshot2.data.value['name'] == 'c_plus') {
                                text = c;
                              } else if (snapshot2.data.value['name'] == 'c_sharp') {
                                text = csharp;
                              }
                              return Column(
                                children: <Widget>[
                                  Container(
                                    ...
                                    child: RaisedButton(
                                      ...
                                      onPressed: () {
                                        Navigator.push(context, MaterialPageRoute(builder: (context) => WeeklyChallenge(text, level, skillId)));
                                      },
                                      child: Container(
